...
void auto_zoom_in(t_point position) {
    zoom_level = 8;
    real_zoom = 8;
    float xcen, ycen;
    ...
    xcen = position.x;
    ycen = position.y;
    float final_zoom_factor = 59.54694473025074;     //1.6667^8
    float x_offset = (trans_coord.init_xright - trans_coord.init_xleft) / final_zoom_factor;
    float y_offset = (trans_coord.init_ybot - trans_coord.init_ytop)/final_zoom_factor;
    trans_coord.xleft = xcen - x_offset;
    trans_coord.xright = xcen + x_offset;
    trans_coord.ytop = ycen - y_offset;
    trans_coord.ybot = ycen + y_offset;

    update_transform();
}

void auto_zoom_in_smart(t_point start, t_point end, t_point position){
    float start_x = start.x;
    float start_y = start.y;
    float end_x = end.x;
    float end_y = end.y;
    float mid_x = position.x;
    float mid_y = position.y;
    float x_diff = start_x-end_x;
    float y_diff = start_y-end_y;
    float x_max = trans_coord.init_xright - trans_coord.init_xleft;
    float y_max = trans_coord.init_ytop - trans_coord.init_ybot;
    float x_factor = abs(x_max / x_diff);
    float y_factor = abs(y_max / y_diff);
    float final_factor;
    if (x_factor > y_factor)
        final_factor = x_factor;
    else
        final_factor = y_factor;
    if (final_factor > 60){
        zoom_fit_helper();
        zoom_level = 8;
        real_zoom = 8;
    ...
    update_transform();
        ...
        xcen = position.x;
        ycen = position.y;
        float final_zoom_factor = 12.861368364198562;     //1.6667^4
        float x_offset = (trans_coord.init_xright - trans_coord.init_xleft) / final_zoom_factor;
        float y_offset = (trans_coord.init_ybot - trans_coord.init_ytop)/final_zoom_factor;
        trans_coord.xleft = xcen - x_offset;
        trans_coord.xright = xcen + x_offset;
...
void auto_zoom_in2(t_point position) {
    zoom_level = 10;
    real_zoom = 10;
    float xcen, ycen;
    ...
    xcen = position.x;
    ycen = position.y;
    float final_zoom_factor = 165.41479619960;      //1.6667^10
    float x_offset = (trans_coord.init_xright - trans_coord.init_xleft) / final_zoom_factor;
    float y_offset = (trans_coord.init_ybot - trans_coord.init_ytop)/final_zoom_factor;
    trans_coord.xleft = xcen - x_offset;
    trans_coord.xright = xcen + x_offset;
...
void auto_zoom_in_street(t_point position) {
    zoom_level = 6;
    real_zoom = 6;
    float xcen, ycen;
    ...
    xcen = position.x;
    ycen = position.y;
    float final_zoom_factor = 21.436042652609739420439369;        //1.6667^6
    float x_offset = (trans_coord.init_xright - trans_coord.init_xleft) / final_zoom_factor;
    float y_offset = (trans_coord.init_ybot - trans_coord.init_ytop)/final_zoom_factor;
    trans_coord.xleft = xcen - x_offset;
    trans_coord.xright = xcen + x_offset;
...
void initial_zoom_in(){
   float xcen, ycen;
    ...
    trans_coord.xleft = xcen - (xcen - trans_coord.xleft) / ZOOM_FACTOR;
    trans_coord.xright = xcen + (trans_coord.xright - xcen) / ZOOM_FACTOR;
    //make ybot - ytop = 0.6 of the original distance
    trans_coord.ytop = ycen - (ycen - trans_coord.ytop) / ZOOM_FACTOR;
    trans_coord.ybot = ycen + (trans_coord.ybot - ycen) / ZOOM_FACTOR;
    update_transform();
}
...
zoom_out(void (*drawscreen) (void)) {
   ...
   float temp_top,temp_bot,temp_left,temp_right;
    xcen = (trans_coord.xright + trans_coord.xleft) / 2;
    ycen = (trans_coord.ybot + trans_coord.ytop) / 2;
    temp_left = xcen - (xcen - trans_coord.xleft)* ZOOM_FACTOR;
    temp_right = xcen + (trans_coord.xright - xcen)* ZOOM_FACTOR;
    temp_top = ycen - (ycen - trans_coord.ytop)* ZOOM_FACTOR;
    temp_bot = ycen + (trans_coord.ybot - ycen)* ZOOM_FACTOR;
    if(temp_top<=map_top&&temp_bot>=map_bottom&&temp_left>=map_left&&temp_right<=map_right){
...
translate_up(void (*drawscreen) (void)) {
    ...
    float temp_top=trans_coord.ytop- ystep;
    float temp_bottom=trans_coord.ybot-ystep;
    if(temp_top<=map_top){
...
translate_left(void (*drawscreen) (void)) {
    ...
    xstep = (trans_coord.xright - trans_coord.xleft) / 2;
     float temp_left=  trans_coord.xleft - xstep;
     if(temp_left>=map_left){
...
translate_right(void (*drawscreen) (void)) {
    ...
    xstep = (trans_coord.xright - trans_coord.xleft) / 2;
     float temp_right=  trans_coord.xright + xstep;
     if(temp_right<=map_right){
    trans_coord.xleft += xstep;
    trans_coord.xright += xstep;
    update_transform();
    drawscreen();
...
panning_execute(int x, int y, void (*drawscreen) (void)) {
    ...
    float temp_left,temp_right,temp_top,temp_bottom;
     temp_left= trans_coord.xleft-x_change_world;
    temp_right= trans_coord.xright -x_change_world;
    temp_bottom=trans_coord.ybot- y_change_world;
    temp_top= trans_coord.ytop - y_change_world;
    if(temp_bottom>=map_bottom&&temp_top<=map_top&&temp_left>=map_left&&temp_right<=map_right){
...
//    menu_attributes.event_mask = 0;
   ...
   // strcpy(button_state.button[5].text, "Zoom Out");
    strcpy(button_state.button[4].text, "Zoom Fit");
    strcpy(button_state.button[5].text, "Window");
    //strcpy(button_state.button[6].text, "---1");
    strcpy(button_state.button[6].text, "PostScript");
    strcpy(button_state.button[7].text, "Proceed");
    strcpy(button_state.button[8].text, "Exit");

    //button_state.button[4].fcn = zoom_in;
   // button_state.button[5].fcn = zoom_out;
    button_state.button[4].fcn = zoom_fit;
    button_state.button[5].fcn = adjustwin; // see 'adjustButton' below in WIN32 section
   // button_state.button[6].fcn = NULL;
    button_state.button[6].fcn = postscript;
    button_state.button[7].fcn = proceed;
    button_state.button[8].fcn = quit;
...
void getSurface_dimention_needed(float left, float right, float top, float bottom){
int left_x_bit=xworld_to_scrn(left);
}
...
int xworld_to_scrn_helper(float worldx){
 return static_cast<int> (xworld_to_scrn_fl(worldx) + 0.5);
}
int yworld_to_scrn_helper(float worldy){
    return static_cast<int> (yworld_to_scrn_fl(worldy) + 0.5);
}
